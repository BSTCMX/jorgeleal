...
async function optimizePosters() {
  if (!existsSync(postersDir)) {
    console.log('⚠️  Posters directory not found, skipping...');
    return;
  }
  ...
  if (webpFiles.length === 0) {
    console.log('ℹ️  No poster files to optimize');
    return;
  }

  console.log(`\n📸 Optimizing ${webpFiles.length} poster(s)...\n`);
  ...
  for (const file of webpFiles) {
    ...
    try {
      ...
      await sharp(filePath)
        ...
        .webp({
          quality: 60, // Optimizado para reducir tamaño sin pérdida perceptible
          effort: 6,
          smartSubsample: true
        })
        .toFile(mobilePath);
      ...
      console.log(`  ✓ ${baseName}-256.webp: ${mobileStats.width}x${mobileStats.height} (${(mobileSize / 1024).toFixed(1)} KiB)`);
      ...
      console.log(`  ✓ ${baseName}-512.webp: ${desktopStats.width}x${desktopStats.height} (${(desktopSize / 1024).toFixed(1)} KiB)`);
